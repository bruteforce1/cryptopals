...
Under the hood, have the function append 5-10 bytes (count choseni
...
def gen_random_key(block=16):
    if not 1 <= block <= 32:
        return b''
    rands = []
    for x in range(1,block):
        rands.append(random.randint(0,255))
    return bytearray(rands)

def encryption_oracle(text):
    bef = gen_random_key(random.randint(5,10))
    af = gen_random_key(random.randint(5,10))
    pt = bef + text + af
    print(pt)
